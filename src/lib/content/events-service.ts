...
function loadEvents(): SchoolEvent[] {
  const data = readJsonFile<SchoolEvent[]>(DATA_FILE, defaultEvents as SchoolEvent[]);
  if (data === defaultEvents) {
...
export async function createEvent(
  input: Omit<SchoolEvent, "id" | "slug"> & { slug?: string },
): Promise<SchoolEvent> {
  const events = loadEvents();
  const slug =
    input.slug?.trim() ||
    input.title.toLowerCase().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").trim();
  const id = `evt-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;

  const event: SchoolEvent = { ...input, id, slug };
  events.push(event);
  writeJsonFile(DATA_FILE, events);
  return event;
}

export async function updateEvent(
  id: string,
  input: Partial<Omit<SchoolEvent, "id">>,
): Promise<SchoolEvent | null> {
  const events = loadEvents();
